...
def calc_kappa(k, kx, gx, a, n0, n1):
...
def calc_coupling_length_vs_gap(
    n0, n1, width, height, gap, lambida, p, q):
    """
    ...
    kappa, deltao, norm_freq = calc_kappa(k, kx, gamma_x, width/2, n0, n1)
...
## assumir um erro de fabricação na diferenca de largura dos guias e ver como isso muda o max power couplinf efficiency.


gaps = np.linspace(20e-9, 400e-9, 50)
Lc = []
for gap in gaps:
    coupling_length = calc_coupling_length_vs_gap(n0, n1, width, height, gap, lambida, p, q)
...
plt.minorticks_on()
plt.savefig("Lc_vs_gap.pdf")
plt.show()

# z = np.linspace(0.0, 10e-6, 1000)
# Pa, Pb = calc_optical_power_evolution(z, kappa, delta)